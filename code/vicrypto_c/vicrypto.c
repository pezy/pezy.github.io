...
void GetLocalNetInterface(char *ifa_name)
{
	...
	{
            if (0 != strcmp("lo", tmp->ifa_name))
            {
    		strncpy(ifa_name, tmp->ifa_name, strlen(tmp->ifa_name));
	        break;
    	    }
	}
	tmp = tmp->ifa_next;
...
void xor_buf(const char in[], char out[], size_t len)
{
    size_t idx;
...
void AddRoundKey(char state[][4], const int w[])
{
    char subkey[4];
...
void aes_encrypt_ctr(const char in[], size_t in_len, char out[], const int key[], int keysize, const char iv[])
{
    ...
    char iv_buf[AES_BLOCK_SIZE], out_buf[AES_BLOCK_SIZE];
...
void Encrypt(char *plaintext, size_t len)
{
    ...
    char iv[16] = {0xf0,0xf1,0xf2,0xf3,0xf4,0xf5,0xf6,0xf7,0xf8,0xf9,0xfa,0xfb,0xfc,0xfd,0xfe,0xff};
    ...
    aes_encrypt_ctr(plaintext, len, enc_buf, key_schedule, 256, iv);

    memcpy(plaintext, enc_buf, len);
}
...
void Decrypt(char *ciphertext, size_t len)
{
    ...
    aes_key_setup(key, key_schedule, 256);
    aes_decrypt_ctr(ciphertext, len, enc_buf, key_schedule, 256, iv);

    memcpy(ciphertext, enc_buf, len);
}
...
void Str2Date(const char *str, int *year, int *mon, int *day)
{
    char sYear[5], sMonth[3], sDay[3];
    memcpy(sYear, str, 4); sYear[4] = '\0';
    memcpy(sMonth, str + 4, 2); sMonth[2] = '\0';
    memcpy(sDay, str + 6, 3); sDay[2] = '\0';
...
void AddMonths(const char *before, int nmonths, char *after)
{
    if (!nmonths)
        return;

    int y, m, d;
    Str2Date(before, &y, &m, &d);

    int old_y = y;
    int increasing = nmonths > 0;
    ...
    int dayInMonth = DaysInMonth(m, y);
    if (dayInMonth < d)
        d = dayInMonth;
    ...
    strftime(after, 9, "%Y%m%d", &date);
}

void License2Str(struct LicenseInfo *lic, char **str, size_t *size)
{
    *size = strlen(lic->app_product_number) + strlen(lic->username) + strlen(lic->nodename)
            + strlen(lic->MAC) + 1 + strlen(lic->start_time) + strlen(lic->stop_time);
    *size += 7;

    *str = (char*)malloc(*size);
    snprintf(*str, *size, "%s|%s|%s|%s|%d|%s|%s", lic->app_product_number, lic->username, lic->nodename, lic->MAC, lic->limit, lic->start_time, lic->stop_time);
    (*str)[*size - 1] = '\0';
}

void Str2License(const char *str, struct LicenseInfo **lic)
{
    *lic = (struct LicenseInfo*)malloc(sizeof(struct LicenseInfo));
    memset(*lic, 0, sizeof(struct LicenseInfo));

    sscanf(str, "%[^|]|%[^|]|%[^|]|%[^|]|%d|%[^|]|%s", (*lic)->app_product_number, (*lic)->username, (*lic)->nodename, (*lic)->MAC, &(*lic)->limit, (*lic)->start_time, (*lic)->stop_time);
}

void AddNodeToLicenseFile(const char *app_product_number, int nMonths)
{
    struct LicenseInfo newNode;
    memset(&newNode, 0, sizeof(struct LicenseInfo));

    strncpy(newNode.app_product_number, app_product_number, strlen(app_product_number));
    GetLocalUserName(newNode.username, sizeof(newNode.username));
    GetLocalNodeName(newNode.nodename, sizeof(newNode.nodename));
    GetLocalMACAddress(newNode.MAC);

    time_t now = time(NULL);
    struct tm *ptm = localtime(&now);
    strftime(newNode.start_time, 9, "%Y%m%d", ptm);
    ...
    {
        newNode.limit = 1;
        AddMonths(newNode.start_time, nMonths, newNode.stop_time);
    }

    char *info = NULL;
    size_t n;
    License2Str(&newNode, &info, &n);

    printf("add: %s\n", info);
    Encrypt(info, n - 1);

    struct FileFormat node_line;
    memset(&node_line, 0, sizeof(struct FileFormat));

    node_line.size = (short)n;
    memcpy(node_line.lic, info, n);

    FILE *file = fopen(LICENSE_FILENAME, "a+b");
    if (file)
    {
        fwrite(&node_line, sizeof(struct FileFormat), 1, file);
        fclose(file);
    }
...
time_t Str2time_t(const char *str)
{
    ...
    Str2Date(str, &tmStart.tm_year, &tmStart.tm_mon, &tmStart.tm_mday);
...
{
    char username[32], nodename[64], MAC[18];
    GetLocalUserName(username, sizeof(username));
    GetLocalNodeName(nodename, sizeof(nodename));
    GetLocalMACAddress(MAC);

    time_t now = time(NULL);
    localtime(&now);

    FILE *file = fopen(LICENSE_FILENAME, "rb");
    if (!file)
    {
        printf("Cannot find the License.\n");
        printf("Username: %s\nNodename: %s\nMAC: %s\n", username, nodename, MAC);
        return 0;
    }
    ...
    while (fread(&node_line, sizeof(struct FileFormat), 1, file) == 1)
    {
        Decrypt(node_line.lic, node_line.size);
        node_line.lic[node_line.size - 1] = '\0';

        printf("check: %s\n", node_line.lic);

        struct LicenseInfo* lic = NULL;
        Str2License(node_line.lic ,&lic);

        int pass = 1;
        pass = pass && !strcmp(lic->username, username);
        pass = pass && !strcmp(lic->nodename, nodename);
        pass = pass && !strcmp(lic->MAC, MAC);

        if (lic->limit)
        {
            time_t start_time = Str2time_t(lic->start_time);
            time_t stop_time = Str2time_t(lic->stop_time);

            pass = pass && (difftime(now, start_time) > 0);
            pass = pass && (difftime(stop_time, now) > 0);
        }
    ...
    printf("Sorry, the product is not authorized for use on this machine.\n");
    printf("Username: %s\nNodename: %s\nMAC: %s\n", username, nodename, MAC);
    fclose(file);
